...
- **Workouts (`workout.py`)** — planned workout upload (intervals.icu → iGPSPORT). `upload_workouts(WorkoutUploadConfig, progress)` fetches the live iGPSPORT custom-workout list to validate dedup, then fetches calendar events and pushes custom workouts via the iGPSPORT mobile JSON API. UI-free; used by GUI and CLI.
